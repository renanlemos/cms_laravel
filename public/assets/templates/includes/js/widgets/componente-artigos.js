	...
	return{
		...
		link: function(scope,element,attr,model){
			...
			request.query(function(response){
			 ...
			 var template =  "";
			 if(scope.artigo_parametros.template == undefined){
			 	template = 'padrao.html'
			 }else{
			 	template = scope.artigo_parametros.template;
			 }
			 var templateUrl = PATH_TEMPLATES+'componentes/artigo/unico-artigo/'+template;
			...
			if($routeParams.apelido2 != undefined){
				request_artigo.query(function(response){
					if(response.length > 0){
						...
						var template = parametros.categoria_template.substr(0,length_template)+".html";

						var templateUrl = PATH_TEMPLATES+'componentes/artigo/categoria-de-artigo/detalhe/'+template;
						var html = "<div ng-include src=\"'"+templateUrl+"'\" />";
						var el = angular.element(html);
						var compile = $compile(el)(scope);
						element.html(compile);
				...
				request.query(function(response){
				 ...
				 var length_template = parametros.categoria_template.length - 5;
				 scope.artigos = response[0].artigos;
				 ...
				 var templateUrl = PATH_TEMPLATES+'componentes/artigo/categoria-de-artigo/categoria/'+scope.categoria_parametros.template_categoria;
				 var html = "<div ng-include src=\"'"+templateUrl+"'\" />";
				 var el = angular.element(html);
				 var compile = $compile(el)(scope);
				 element.html(compile);
