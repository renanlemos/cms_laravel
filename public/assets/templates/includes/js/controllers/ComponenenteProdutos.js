$app.controller('ComponenteProdutosController',function($timeout,$scope,$location,$resource,$filter,$compile){
    ...
    pagination = function(){
	    ...
	    $scope.$watch(function () {
	    	$scope.filtered = $scope.$eval("produtos | filter:search");
    		if(($scope.categoria_parametros.paginacao)&&($scope.categoria_parametros.formulario_busca)){
    			...
    			$scope.repeat = $scope.$eval("produtos | startFrom:(currentPage-1)*categoria_parametros.paginacao_itens_por_pagina | limitTo: categoria_parametros.paginacao_itens_por_pagina");
    		}else{
    			$scope.repeat = $scope.$eval("produtos");
    		}
		});
	}
