$app.controller('ModuloProdutoDestaquesController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){

	$scope.title = "Módulo Produto em Destaque";
    ...
    $scope.window = {
    	new: function(){
            ...
            $scope.dataItem = {titulo:"",apelido:"",tipo:"produto-em-destaque",modulo_posicao_id:"",publicado:true,mostrar_titulo:0,tag_titulo:"h1",conteudo:"",parametros:{categoria_id:"",template:"",tipo:"categoria",produtos:[],class:"",id:""},ordem:""};
    ...
    create = function(){
        ...
        if(AuthService.checkCreate("modulo-produto-destaques",true) && AuthService.checkCreate("modulos",true) && AuthService.checkCreate("modulo-produtos",true)){
    ...
    update = function(){
        ...
        if(AuthService.checkUpdate("modulo-produto-destaques",true) && AuthService.checkUpdate("modulos",true) && AuthService.checkUpdate("modulo-produtos",true)){
        	var request= $resource("/admin/modulos",{_token:CSRF_TOKEN},{
        		update: {method:"PUT"}
        	});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("modulo-produto-destaques",true) && AuthService.checkDelete("modulos",true) && AuthService.checkDelete("modulo-produtos",true)){
    ...
    optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/modulos/grid?tipo=produto-em-destaque'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
    ...
    optionsCombo = function(){
        ...
        $scope.comboTipo = {
            ...
            autoBind: false,
            dataSource:{
                data: [{text:"Produtos de Categoria",value:"categoria"},{text:"Produtos",value:"produto"}]
            }
        }
        var pasta = "/assets/templates/includes/templates/modulos/produtos/produto-em-destaques";
        $scope.comboTemplate = {
            filter: 'contains',
            dataTextField: "text",
            dataValueField: "value",
        ...
        $scope.comboProdutos = {
            ...
            change: function(e){
                ...
                if(search == false){
                    ...
                    var produto = {id:this.value(),nome:this.text()};
                    if(produtoInArray(produto)){
                        alert('- Esse produto já foi selecionado.');
                        return  false;
                    }
                    $scope.dataItem.parametros.produtos.push(produto);
                    this.value("");
                    $scope.$apply();
    ...
    var produtoInArray = function(produto){
        var resp = false;   
        if($scope.dataItem.parametros.produtos.length > 0){
            angular.forEach($scope.dataItem.parametros.produtos,function(v,k){
                if(resp == false){
                    if(v.id == produto.id){
                        resp = true;
                    }
    ...
    $scope.deleteProduto = function(index){
        $scope.dataItem.parametros.produtos.splice(index,1);
    }
	...
	init = function(){
        ...
        AuthService.checkView("modulo-produtos");
		AuthService.checkView("modulo-produto-destaques");
