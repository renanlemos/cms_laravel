$app.controller('ModuloMenuController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){

	$scope.title = "Módulo Menu";
    ...
    $scope.window = {
    	new: function(){
            ...
            $scope.dataItem = {titulo:"",apelido:"",tipo:"menu",modulo_posicao_id:"",publicado:true,mostrar_titulo:0,tag_titulo:"h1",conteudo:"",parametros:{menu_id:"",menu_template:"padrao",class:"",id:""},ordem:""};
    ...
    create = function(){
        ...
        if(AuthService.checkCreate("modulo-menu",true) && AuthService.checkCreate("modulos",true)){
    ...
    update = function(){
        ...
        if(AuthService.checkUpdate("modulo-menu",true) && AuthService.checkUpdate("modulos",true)){
        	var request= $resource("/admin/modulos",{_token:CSRF_TOKEN},{
        		update: {method:"PUT"}
        	});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("modulo-menu",true) && AuthService.checkDelete("modulos",true)){
    ...
    optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/modulos/grid?tipo=menu'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
    ...
    optionsCombo = function(){
        ...
        $scope.comboMenuTemplate = {
            filter: 'contains',
            dataTextField: "text",
            dataValueField: "value",
            autoBind: false,
            dataSource:{
                data: [{text:"Padrão",value:'padrao'}]
            }
        }
    }
    ...
    refreshComboPosicao = function(){
        var comboPosicao = $("#comboPosicao").data("kendoComboBox"); 
        comboPosicao.dataSource.read();
    }
    refreshComboMenu = function(){
        var comboPosicao = $("#comboMenu").data("kendoComboBox"); 
        comboPosicao.dataSource.read();
    }
	...
	init = function(){
		...
		AuthService.checkView("modulo-menu");
