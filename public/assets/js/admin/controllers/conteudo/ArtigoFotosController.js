$app.controller("ArtigoFotosController",function($scope,$rootScope,$resource,AuthService,FileService,CSRF_TOKEN){
    ...
    $scope.window = {
    	new: function(){
            ...
            $scope.dataItem = {titulo:"",alt:"",legenda:"",imagem:"",publicado:true,capa:false,artigo_id:$scope.artigo.id};
            $scope.formView = "new";
        ...
        ordem: function(){
            $scope.paramsOrdem = {artigo_id:$scope.artigo.id};
    ...
    create = function(){
        if(AuthService.checkCreate("artigos",true) && AuthService.checkCreate("conteudo",true)){
            var user = $resource("/admin/conteudo/artigos/artigo-fotos",{_token:CSRF_TOKEN});
    ...
    update = function(){
        ...
        if(AuthService.checkCreate("conteudo",true) && AuthService.checkUpdate("artigos",true) && AuthService.checkUpdate("componentes",true)){
            var request = $resource("/admin/conteudo/artigos/artigo-fotos",{_token:CSRF_TOKEN},{
                update: {method:"PUT"}
            });
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkCreate("conteudo",true) && AuthService.checkDelete("artigos",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
                ...
                var user = $resource("/admin/conteudo/artigos/artigo-fotos");
	...
	$scope.optionsGrid = function(dataItem){
        ...
        return{
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/conteudo/artigos/artigo-fotos/grid?artigo_id='+dataItem.id
		    },
		    serpageSize: 10,
		    serverPaging: true, 
		...
		toolbar:[{template:"<button class='k-button' k-options='optionsTooltip' kendo-tooltip title='Adicionar' ng-click='window.new()'><span class='k-icon k-i-plus' style='padding:0 2px 0 0;'></span></button>"},
                ...
                {template:"<button class='k-button' k-options='optionsTooltip' kendo-tooltip title='Voltar para Artigos' ng-click='$parent.setContent(\"artigos\")'>Voltar</button>"},
        ...
        {field: "imagem", title: "Imagem",type:"string",template:"<div class='text-center'><img src='/conteudo/artigos/artigo-#=artigo_id#/#=imagem#' width='50px;'></div>",},
        {field: "titulo", title: "Titulo",type:"string"},
        {field: "publicado",width: '100px',title: "Publicado",template:"<publica-in-grid campo='publicado' status='dataItem.publicado' item='dataItem' grid='grid' url='/admin/conteudo/artigos/artigo-fotos/updateStatus' permissao='permissaoUpdate'></publica-in-grid>",type:"boolean"},
        {field: "capa",width: '100px',title: "Capa",template:"<publica-in-grid campo='capa' status='dataItem.capa' item='dataItem' grid='grid' url='/admin/conteudo/artigos/artigo-fotos/updateStatus' permissao='permissaoUpdate'></publica-in-grid>",type:"boolean"},
    ...
    $scope.uploadFiles = function(file){
        var path = 'conteudo/artigos/artigo-'+$scope.artigo.id;
    ...
    $scope.updateOrdem = function(){
        ...
        var request = $resource('/admin/conteudo/artigos/artigo-fotos/ordem',{_token:CSRF_TOKEN},{
            update:{method:"PUT"}
        }); 
    ...
    optionsCombo = function(dataItem){
        $scope.comboImagem = {
            ...
            dataSource:{
                transport: {
                    read: {
                        url: "/admin/files/read_dir?pasta=conteudo/artigos/artigo-"+dataItem.id,
                        async: false
                    }
                }
            },
            template:"<img src='/conteudo/artigos/artigo-"+dataItem.id+"/#=value#' width='100px;'>",
            change: function(e){
