$app.controller('EventoContatosController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){

	$scope.title = "Contato de Eventos";
    $scope.editorDescricao = false;

    $scope.window = {
    	new: function(){
            $scope.openModal = true;
            $scope.dataItem = {nome:"",email:"",telefone:"",ramal:"",descricao:""};
            $scope.formView = "new";
    		$scope.formAction = "create()";
    	    $scope.modal.setOptions({
    	    	title: "Contato de Evento"
            });
    	...
    	edit: function(dataItem){
    		...
    		$scope.formAction = "update()";
    		 $scope.modal.setOptions({
    	    	title: "Editar Contato de Evento: "+dataItem.id
            });
    ...
    create = function(){
        if(AuthService.checkCreate("componente-eventos",true) && AuthService.checkCreate("componente-evento-categorias",true) && AuthService.checkCreate("componentes",true)){
          	var request = $resource("/admin/componentes/eventos/evento-contatos",{_token:CSRF_TOKEN});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-eventos",true) && AuthService.checkDelete("componente-evento-categorias",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
            	...
            	var request= $resource("/admin/componentes/eventos/evento-contatos");
    ...
    optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/componentes/eventos/evento-contatos/grid'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
