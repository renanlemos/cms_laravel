$app.controller('BannerSlidersController',function($scope,$http,$window,$location,$resource,AuthService,CSRF_TOKEN){
	
	$scope.title = "Banner Slider";
	$scope.content = "banner";
	...
	$scope.window = {
    	...
    	edit: function(dataItem){
    		...
    		$scope.formAction = "update()";
    		 $scope.modal.setOptions({
    	    	title: "Editar Banner: "+dataItem.id
            });
    		$scope.modal.open().center();
    ...
    createBanner = function(){
        if(AuthService.checkCreate("componente-banner-slider",true) && AuthService.checkCreate("componentes",true)){
            var request = $resource("/admin/componentes/banner-sliders",{_token:CSRF_TOKEN});
    ...
    updateBanner = function(){
        ...
        if(AuthService.checkUpdate("componente-banner-slider",true) && AuthService.checkUpdate("componentes",true)){
            var user = $resource("/admin/componentes/banner-sliders",{_token:CSRF_TOKEN},{
                update: {method:"PUT"}
            });
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-banner-slider",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
                ...
                var user = $resource("/admin/componentes/banner-sliders");
	...
	optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/componentes/banner-sliders/grid'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
        ...
        {command: [
           ...
           {template: "<button  class=\'k-button\' k-options='optionsTooltip' kendo-tooltip title='Imagens' ng-click=\'setContent(\"imagens\",dataItem)\'><span class='icon-image-2' style='padding:0 2px 0 0;'></span></button>"}
	    ]}] 
    	}
    }
    ...
    $scope.setContent = function(content,dataItem){
        ...
        if(content == 'imagens'){
            $scope.title = "Banner: "+dataItem.descricao;  
        }else{
            $scope.title = "Banner Slider";
        }
    ...
    init = function(){
		...
		AuthService.checkView("componente-banner-slider"); 
