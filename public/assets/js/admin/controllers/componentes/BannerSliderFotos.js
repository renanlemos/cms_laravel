$app.controller("BannerSliderFotosController",function($scope,$rootScope,$resource,AuthService,FileService,CSRF_TOKEN){
    ...
    $scope.window = {
    	new: function(){
            ...
            $scope.dataItem = {titulo:"",alt:"",legenda:"",imagem:"",publicado:true,url:"",url_target:"_self",banner_slider_id:$scope.banner.id};
            $scope.formView = "new";
        ...
        ordem: function(){
            $scope.paramsOrdem = {banner_slider_id:$scope.banner.id};
    ...
    create = function(){
        if(AuthService.checkCreate("componente-banner-slider",true) && AuthService.checkCreate("componentes",true)){
            var user = $resource("/admin/componentes/banner-slider-fotos",{_token:CSRF_TOKEN});
            ...
            user.save($scope.dataItem,function(response){
                switch(response.status){
                    ...
                    case 1:
                        alert(response.data);
                         $scope.dataItem = {titulo:"",alt:"",legenda:"",imagem:"",publicado:true,banner_slide_id:$scope.banner.id};
    ...
    update = function(){
        ...
        if(AuthService.checkUpdate("componente-banner-slider",true) && AuthService.checkUpdate("componentes",true)){
            var request = $resource("/admin/componentes/banner-slider-fotos",{_token:CSRF_TOKEN},{
                update: {method:"PUT"}
            });
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-banner-slider",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
                ...
                var user = $resource("/admin/componentes/banner-slider-fotos");
	...
	$scope.optionsGrid = function(dataItem){
        ...
        return{
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/componentes/banner-slider-fotos/grid?banner_slider_id='+dataItem.id
		    },
		    serpageSize: 10,
		    serverPaging: true, 
		...
		toolbar:[{template:"<button class='k-button' k-options='optionsTooltip' kendo-tooltip title='Adicionar' ng-click='window.new()'><span class='k-icon k-i-plus' style='padding:0 2px 0 0;'></span></button>"},
                ...
                {template:"<button class='k-button' k-options='optionsTooltip' kendo-tooltip title='Voltar para Banners' ng-click='$parent.setContent(\"banner\")'>Voltar</button>"},
        ...
        {field: "imagem", title: "Imagem",type:"string",template:"<div class='text-center'><img src='/componentes/banner-slider/banner-#=banner_slider_id#/#=imagem#' width='100px;'></div>",},
    ...
    $scope.uploadFiles = function(file){
        var path = 'componentes/banner-slider/banner-'+$scope.banner.id;
    ...
    $scope.updateOrdem = function(){
        ...
        var request = $resource('/admin/componentes/banner-slider-fotos/ordem',{_token:CSRF_TOKEN},{
            update:{method:"PUT"}
        }); 
        if(AuthService.checkUpdate("componente-banner-slider",true) && AuthService.checkUpdate("componentes",true)){
    ...
    optionsCombo = function(dataItem){
        $scope.comboImagem = {
            ...
            dataSource:{
                transport: {
                    read: {
                        url: "/admin/files/read_dir?pasta=componentes/banner-slider/banner-"+dataItem.id,
                        async: false
                    }
                }
            },
            template:"<img src='/componentes/banner-slider/banner-"+dataItem.id+"/#=value#' width='100px;'>",
            change: function(e){
