$app.controller('EventoCategoriasController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){

	$scope.title = "Categoria de Eventos";
    ...
    create = function(){
        if(AuthService.checkCreate("componente-eventos",true) && AuthService.checkCreate("componente-evento-categorias",true) && AuthService.checkCreate("componentes",true)){
          	var request = $resource("/admin/componentes/eventos/evento-categorias",{_token:CSRF_TOKEN});
    ...
    update = function(){
        ...
        if(AuthService.checkUpdate("componente-eventos",true) && AuthService.checkUpdate("componente-evento-categorias",true) && AuthService.checkUpdate("componentes",true)){
        	var request= $resource("/admin/componentes/eventos/evento-categorias",{_token:CSRF_TOKEN},{
        		update: {method:"PUT"}
        	});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-eventos",true) && AuthService.checkDelete("componente-evento-categorias",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
            	...
            	var request= $resource("/admin/componentes/eventos/evento-categorias");
    ...
    $scope.updateOrdem = function(){
        ...
        var request = $resource('/admin/componentes/eventos/evento-categorias/ordem',{_token:CSRF_TOKEN},{
            update:{method:"PUT"}
        }); 
        if(AuthService.checkUpdate("componente-eventos",true) && AuthService.checkUpdate("componente-evento-categorias",true) && AuthService.checkUpdate("componentes",true)){
    ...
    optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/componentes/eventos/evento-categorias/grid'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
		...
		toolbar:[{template:"<button class='k-button' k-options='optionsTooltip' kendo-tooltip title='Adicionar' ng-disabled='access_create == false' ng-click='window.new()'><span class='k-icon k-i-plus' style='padding:0 2px 0 0;'></span></button>"},
                ...
                {template:"<button class='k-button' k-options='optionsTooltip' kendo-tooltip title='Ordenação' ng-click='window.ordem()'><span class='k-icon' style='padding:0 2px 0 0;'></span>Ordem</button>"}
        ], 	        
	    columns:[
        ...
        {field: "publicado",width:'100px',title: "Publicado",template:"<publica-in-grid campo='publicado' status='dataItem.publicado' item='dataItem' grid='grid' url='/admin/componentes/eventos/evento-categorias/updateStatus' permissao='permissaoUpdate'></publica-in-grid>",type:"boolean",
            filterable:{
                extra: false,
                operators:{string:{
                    eq: "Seja igual a",
    ...
    optionsCombo = function(){
         var pathTemplates = '/assets/templates/includes/templates/componentes/evento/categoria-de-evento/';
    ...
    init = function(){
        ...
        AuthService.checkView("componente-evento-categorias");

        $scope.permissaoUpdate =  AuthService.checkUpdate("componente-eventos",false) == true && AuthService.checkUpdate("componente-evento-categorias",false) == true && AuthService.checkUpdate("componentes",false) == true ? true : false;
