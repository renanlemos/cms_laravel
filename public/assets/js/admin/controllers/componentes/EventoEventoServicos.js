$app.controller('EventoEventoServicosController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){
    ...
    $scope.window = {
        new: function(){
            ...
            $scope.dataItem = {evento_id:$scope.evento.id,evento_servico_id:"",publicado:true};
            $scope.formView = "new";
        ...
        edit: function(dataItem){
            ...
            $scope.formAction = "update()";
             $scope.modal.setOptions({
                title: "Editar Sertviço: "+dataItem.id
            });
    ...
    createS = function(){
        if(AuthService.checkCreate("componente-eventos",true) && AuthService.checkCreate("componente-evento-eventos",true) && AuthService.checkCreate("componentes",true)){
            var request = $resource("/admin/componentes/eventos/evento-evento-servicos",{_token:CSRF_TOKEN});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-eventos",true) && AuthService.checkDelete("componente-evento-eventos",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
                ...
                var request= $resource("/admin/componentes/eventos/evento-evento-servicos");
    ...
    $scope.optionsGrid = function(dataItem){
        ...
        return $scope.optionsGrid = {
            ...
            type: "json",
            transport:{ 
              read: '/admin/componentes/eventos/evento-evento-servicos/grid?evento_id='+dataItem.id
            },
            serpageSize: 10,
            serverPaging: true, 
            ...
            schema:{ 
              data: function(response){
                dataServicosGrid = response.servicos;
        ...
        {field: "evento_servico_id", title: "Serviço",type:"number",template:function(dataItem){return ListDataService.getForeignText(dataServicosGrid,dataItem.evento_servico_id);},filterable:{ui:servicosFilter,extra:false,operators:{number:{eq:"Seja igual a"}}}},
        {field: "publicado",width:'100px',title: "Publicado",template:"<publica-in-grid campo='publicado' status='dataItem.publicado' item='dataItem' grid='grid' url='/admin/componentes/eventos/evento-evento-servicos/updateStatus?campo=publicado' permissao='permissaoUpdate'></publica-in-grid>",type:"boolean",
            filterable:{
                extra: false,
                operators:{string:{
                    eq: "Seja igual a",
        ...
        function servicosFilter(element){
            element.kendoDropDownList({
                dataTextField: "text",
                dataValueField: "value",
                dataSource:{data:dataServicosGrid},
                optionLabel: "--Select Value--"
            });
        }
    }
    $scope.updateOrdem = function(){
        ...
        var request = $resource('/admin/componentes/eventos/evento-evento-servicos/ordem',{_token:CSRF_TOKEN},{
            update:{method:"PUT"}
        }); 
        if(AuthService.checkUpdate("componente-eventos",true) && AuthService.checkUpdate("componente-evento-eventos",true) && AuthService.checkUpdate("componentes",true)){
    ...
    optionsCombo = function(){
       $scope.comboServico = {
            ...
            dataSource:{
                transport: {
                    read: {
                        url: '/admin/componentes/eventos/evento-servicos/combo',
                        async: false
                    }
                }
            },
