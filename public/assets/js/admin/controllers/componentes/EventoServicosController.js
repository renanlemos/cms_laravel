$app.controller('EventoServicosController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){

	$scope.title = "Serviços de Eventos";
    ...
    $scope.window = {
    	...
    	edit: function(dataItem){
    		...
    		$scope.formAction = "update()";
    		 $scope.modal.setOptions({
    	    	title: "Editar Serviço: "+dataItem.id
            });
    ...
    create = function(){
        if(AuthService.checkCreate("componente-eventos",true) && AuthService.checkCreate("componente-evento-servicos",true) && AuthService.checkCreate("componentes",true)){
          	var request = $resource("/admin/componentes/eventos/evento-servicos",{_token:CSRF_TOKEN});
    ...
    update = function(){
            ...
            if(AuthService.checkUpdate("componente-eventos",true) && AuthService.checkUpdate("componente-evento-servicos",true) && AuthService.checkUpdate("componentes",true)){
        	var request= $resource("/admin/componentes/eventos/evento-servicos",{_token:CSRF_TOKEN},{
        		update: {method:"PUT"}
        	});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-eventos",true) && AuthService.checkDelete("componente-evento-servicos",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
            	...
            	var request= $resource("/admin/componentes/eventos/evento-servicos");
    ...
    optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/componentes/eventos/evento-servicos/grid'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
    ...
    init = function(){
        ...
        AuthService.checkView("componente-evento-servicos");
