$app.controller('ProdutoCaracteristicaTiposController',function($timeout,$scope,$rootScope,$resource,AuthService,ListDataService,CSRF_TOKEN){

	$scope.title = "Tipo de Caracteristica";
    $scope.openOrdem = false;

    $scope.window = {
    	new: function(){
            $scope.openModal = true;
            $scope.dataItem = {descricao:"",publicado:true};
            $scope.formView = "new";
    	...
    	edit: function(dataItem){
    		...
    		$scope.formAction = "update()";
    		 $scope.modal.setOptions({
    	    	title: "Editar Tipo de Categoria: "+dataItem.id
            });
    ...
    create = function(){
        if(AuthService.checkCreate("componente-produtos",true) && AuthService.checkCreate("componente-produto-caracteristica-tipos",true) && AuthService.checkCreate("componentes",true)){
          	var request = $resource("/admin/componentes/produtos/produto-caracteristica-tipos",{_token:CSRF_TOKEN});
    ...
    update = function(){
        ...
        if(AuthService.checkUpdate("componente-produtos",true) && AuthService.checkUpdate("componente-produto-caracteristica-tipos",true) && AuthService.checkUpdate("componentes",true)){
        	var request= $resource("/admin/componentes/produtos/produto-caracteristica-tipos",{_token:CSRF_TOKEN},{
        		update: {method:"PUT"}
        	});
    ...
    $scope.delete = function(dataItem){
        ...
        if(AuthService.checkDelete("componente-produtos",true) && AuthService.checkDelete("componente-produto-caracteristica-tipos",true) && AuthService.checkDelete("componentes",true)){
            if(confirm("Deseja deletar esse(s) registro(s)?")){
            	...
            	var request= $resource("/admin/componentes/produtos/produto-caracteristica-tipos");
    ...
    $scope.updateOrdem = function(){
        ...
        var request = $resource('/admin/componentes/produtos/produto-caracteristica-tipos/ordem',{_token:CSRF_TOKEN},{
            update:{method:"PUT"}
        }); 
         if(AuthService.checkCreate("componente-produto-caracteristica-tipos",true) && AuthService.checkUpdate("componente-produtos",true) && AuthService.checkUpdate("componentes",true)){
    ...
    optionsGrid = function(){
        ...
        return $scope.optionsGrid = {
		    ...
		    type: "json",
		    transport:{ 
		      read: '/admin/componentes/produtos/produto-caracteristica-tipos/grid'
		    },
		    serpageSize: 10,
		    serverPaging: true, 
        ...
        {field: "publicado", title: "Publicado",template:"<publica-in-grid campo='publicado' status='dataItem.publicado' item='dataItem' grid='grid' url='/admin/componentes/produtos/produto-caracteristica-tipos/updateStatus' permissao='permissaoUpdate'></publica-in-grid>",type:"boolean",
            filterable:{
                extra: false,
                operators:{string:{
                    eq: "Seja igual a",
    ...
    init = function(){
        ...
        AuthService.checkView("componente-produto-caracteristica-tipos");

        $scope.permissaoUpdate =  AuthService.checkUpdate("componente-produtos",false) == true && AuthService.checkUpdate("componente-produto-caracteristica-tipos",false) == true && AuthService.checkUpdate("componentes",false) == true ? true : false;
